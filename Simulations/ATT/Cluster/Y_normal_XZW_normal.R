...
##   - Estimates ATT instead of ATE
##
##  It also does:
##   - Adds a simple imputation method. 
##     1. estimate the variance of the error sigma^2 in the calibration
##     sample and 
##     2. generate an error variable in a multiple imputation setting
##     following N(0,sigma^2) to be subtracted (or added) to the observed variable X,
##     over N datasets (following their approach to combine across datasets).
...
sampling <- function(cor_level, X_effect, m_error){
  ...
  data.main  <- data.frame(W = W[i.main], T = T[i.main], Z=X_Z[i.main,"Z"], 
                           Y_obs = Y_obs[i.main])
  data.cause <- data.frame(p= expit(logit_T[i.main]), X= X_Z[i.main,"X"], T = T[i.main], Y0 = Y0[i.main], Y1 = Y1[i.main])
...
true_regression <- function(data_main, Xtrue){
  ...
  data_main$wt <- ifelse(data_main$T==1, 1, p_hat/(1-p_hat))
...
simple_imputation <- function(data_main, data_calib, N_data=12*3){
  ...
  for(k in 1:ncol(X_imp)){
    ...
    MI_data$wt <- ifelse(data_main$T==1, 1, p_hat/(1-p_hat))
...
full_simulation <- function(cor_level = "low", X_effect = "large", m_error = "large"){
  ...
  mean_insample     <- mean(data$cause$Y1[data$cause$T==1] - data$cause$Y0[data$cause$T==1]) 
