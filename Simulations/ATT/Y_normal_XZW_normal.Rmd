...
This simulation is to test how the ATT performs with MIEC.

These are results that include a simpler imputation method, in which a random error is sampled from a $N(0,\sigma^2)$, where $\sigma^2$ is the variance of the error in $W$. This method performs similar to the uncongenial method, and both of these perform worse than using a naive approach.
...
setwd("~/GitHub/PS_MIEC/Simulations/ATT/Cluster")
