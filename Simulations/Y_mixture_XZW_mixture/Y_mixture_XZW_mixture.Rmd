---
title: "Results from Y as a mixture and (X,Z,W) as a truncated mixture"
output:
  html_document:
    toc: true
    toc_depth: 2
...
setwd("C:/Users/Yenny/Dropbox/2012 Research/Causal_Research/Code/DavidCode/Simulation/Confidence intervals/Different Distributions/Y_mixture_XZW_mixture")
...
We generate $X,Z$ as follows. We construct $X$ with $f(X_1) \thicksim F(200,100), \quad f(X_2) \thicksim \chi^2(1), \quad f(X_3) \thicksim N(0,1)$, and define $X = (X_1 + - 2X_2+\frac{1}{2}X_3)$ if $X > -4$, else $X = 4$. We then generate $Z$ to have the same distribution and $cor(X,Z) = \rho$.

We use the measurement error model as $f(W \mid T,X,Z) \thicksim N(\beta_0 + \beta_1 X, \sigma^2)$.

To construct potential outcomes that are not normally distributed, we use $f(e_{t1}) \thicksim F(300,20)$, $f(e_{t2}) \thicksim \Gamma(2,2)$ and set $$Y(T) = \Delta T + \delta_x X + \delta_Z Z + e_{T1} - \frac{20}{18} + e_{T2} - 4.$$
...
levels(results_long$metric)[1] <- "Bias" 
...
p <- qplot(reliability, average, data=subset(results_long, metric != "Cov"), linetype=method_relev, color=method_relev, facets=metric~gammaX_F+rho_F, size=I(1.2), geom=c("line"), xlab="Reliability", ylab="") 
...
##  This code was based on 'Y_mixture_XWZ_mixture.R', which 
##  was purled from Y_mixture_XWZ_mixture.Rmd, 
...
##   - We assume that Y continuous non-gaussian distribution and 
##   - We assume that X comes from a truncated mixted distribution
...
colnames(X_Z) <- c("X", "Z")
    ...
    e0 <- rf(n_main+n_calib, 300, 20)-(20/18) + rgamma(n_main+n_calib,shape=2 , scale=2)-4
    e1 <- rf(n_main+n_calib, 300, 20)-(20/18) + rgamma(n_main+n_calib,shape=2 , scale=2)-4

    Y0 <- delta_X*X_Z[,"X"] + delta_Z*X_Z[,"Z"] + e0         
    Y1 <- Delta + delta_X*X_Z[,"X"] + delta_Z*X_Z[,"Z"] + e1
...
save(results,file=paste("Simulation_Y_mixture_XZW_mixture_", i, "-", format(Sys.time(), "%Y%m%d-%H%M"),".Rdata", sep=""))
```
