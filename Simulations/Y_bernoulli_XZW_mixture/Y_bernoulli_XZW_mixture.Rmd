...
Simulation of Method (Only PS Model) with m=12,n=3
========================================================

In this project, I extend the set up of the simulations conducted by Guo, Little and McConnel to include the potential outcomes, a causal effect and propensity score estimation. 

This version includes estimation of confidence intervals using mitools and Reiter's method.

This version uses IPTW and does not fit a correct model for the outcome $Y$.

This version runs all reliabilities.

The methods use different sets of variables to form the joint normal distribution:
* $MIEC_{Ycov}$ uses $T$ as outcome, $Z$ and $Y_{obs}$ as helpful covariates
* $MIEC_{noY}$ uses $T$ as outcome, $Z$ as helpful covariate
* $MIEC_{noT}$ uses $Y$ as outcome, $Z$ as helpful covariate
* $MIEC_{noTY}$ uses no outcome, $Z$ as helpful covariate
...
The causal effect for every person is $\Delta = Y(1)-Y(0)$

We define a logistic regression model for the propensity score $p = P(T=1)$, where $T$ is the indicator of treatment assigned:

$$f(T \mid X,Z,\psi) \thicksim Bernoulli( expit(\gamma_0 + \gamma_X X + \gamma_z Z))$$

$$f(X,Z) \thicksim N_2 \Bigl(\bigl(\begin{smallmatrix} 0 \\ 0 \end{smallmatrix} \bigr), \bigl(\begin{smallmatrix} 1 && \rho \\ \rho && 1\end{smallmatrix}\bigl)\Bigl)$$

and a measurement error model:

$$f(W \mid T,X,Z) \thicksim N(\beta_0 + \beta_1 X, \sigma^2)$$

The distribution of the potential outcomes is:

$$f(Y(T) \mid T,X,Z) \thicksim N(\Delta T + \delta_X X + \delta_Z Z, \tau^2) $$

with the errors of the potential outcomes $Y(1), Y(0)$ are independent, and with equal variance.

Here, $\psi = (\gamma_0, \gamma_X, \gamma_Z, \tau^2)$ and $\gamma = (\gamma_X, \gamma_Z)$. 
...
We will use the functions $expit$ and $logit$ many times:
...
```{r simulations, eval=TRUE}
setwd("C:/Users/Yenny/Dropbox/2012 Research/Causal_Research/Code/DavidCode/Simulation/Confidence intervals/Different Distributions/Y_bernoulli_XZW_mixture")
...
```{r print_table, results='asis',eval=TRUE}
print(xtable(print_table), type = 'html', html.table.attributes = '', include.rownames=FALSE)
```

```{r noModelPS_Bias_RMSE, fig.height=5.5,fig.width=13, dpi=200, fig.path='figure/'}
full_cols <- all_cols
...
Figure for paper:
```{r noModelPS_Noncov_re, fig.height=3.5,fig.width=13, dpi=200, fig.path='figure/'}
full_cols <- all_cols
full_cols[seq(2,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
full_cols[seq(3,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
...
```{r noModelPS_Bias_RMSE_color, fig.height=3.5,fig.width=10, dpi=200, fig.path='figure/'}
full_cols <- all_cols
full_cols[seq(2,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
full_cols[seq(3,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
...
Figure for paper:
```{r noModelPS_Noncov_re_color, fig.height=2.5,fig.width=10, dpi=200, fig.path='figure/'}
full_cols <- all_cols
full_cols[seq(2,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
full_cols[seq(3,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
...
```{r noModelPS_Noncov_all, fig.height=4,fig.width=13, fig.path='figure/'}
full_cols <- all_cols
full_cols[seq(2,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))
full_cols[seq(3,nrow(all_cols),by=3),] <- as.numeric(as.matrix(params_cols))

big_table_4graph <- big_table
wide_vars <- colnames(big_table_4graph)
metrics   <- rownames(big_table_4graph)
rownames(big_table_4graph) <- NULL
results_table <- data.frame(mode = factor("Model + No PS"), full_cols, metric = metrics, big_table_4graph)
...
names(results_long)[2:4] <- c("gammaX", "reliability","rho")
results_long$method <- factor(results_long$method)
#levels(results_long$method) <- c("MIEC (Y_obs,Z)", "MIEC (Z)","MIEC (T,Z)","MIEC (Y_obs,T,Z)","naive","Xtrue")
...
# Scatterplot of reliability vs. non coverage for each combination of gammaX and rho
# in each facet, method is represented by shape and color
p <- qplot(reliability, average, data=subset(results_long, metric == "Cov"), shape=method, color=method,facets=metric~gammaX_F+rho_F, size=I(1.2),
  geom=c("line"), xlab="Reliability", ylab="") 
p + geom_hline(yintercept=95)

results_only_miRe <- with(results_long, results_long[metric == "Cov" & (method == "MIEC_Yout_mi.results" | method == "MIEC_Yout_re.results" | method == "MIEC_noT_mi.results" | method == "MIEC_noT_re.results"),])
  
p <- qplot(reliability, average, data=results_only_miRe, shape=method, color=method,facets=metric~gammaX_F+rho_F, size=I(1.2),
  geom=c("line"), xlab="Reliability", ylab="") 
p + geom_hline(yintercept=95)
...
Bias compared to naive method
------------------------------

Let $bias_{naive} = 100%$. Then, calculate the percentage of bias compared to the naive method. 

```{r}
big_table_perc <- big_table[,-grep("_mi", colnames(big_table))]
big_table_perc <- round(-100 + abs(apply(big_table_perc, 1, function(x) (x-x["Xtrue.results"])/(x["naive.results"]-x["Xtrue.results"]))*100), 0)
```
...
print_table <- cbind(all_cols, rownames(big_table), rep(NA,length(all_cols[,1])),t(big_table_perc))
rownames(print_table) <- NULL
colnames(print_table)[1:5] <- c("$\\gamma_X$", "Reliability", "$\\rho$", rep(".",2))
```

```{r print_table_naive, results='asis'}
print(xtable(print_table), type = 'html', html.table.attributes = '', include.rownames=FALSE)
```

Only bias:
```{r print_table_naive_bias, results='asis'}
print(xtable(print_table[seq(1,nrow(print_table), by=3),]), type = 'html', html.table.attributes = '', include.rownames=FALSE)
```

Only RMSE:
```{r print_table_naive_rmse, results='asis'}
print(xtable(print_table[seq(3,nrow(print_table), by=3),]), type = 'html', html.table.attributes = '', include.rownames=FALSE)
```

Only Coverage:
```{r print_table_naive_cov, results='asis'}
print(xtable(print_table[seq(3,nrow(print_table), by=3),]), type = 'html', html.table.attributes = '', include.rownames=FALSE)
```
